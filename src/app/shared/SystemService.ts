...
export class SystemService {
   ...
   // public Account: Account_Model;
    ...
    showMessage(type: AlertType, message: any) {
    ...
    refreshKendoEditor(id: any) { $(id).data("kendoEditor").refresh(); }
...
export class Deferred<T> {
    ...
    constructor() {
        this.promise = new Promise<T>((resolve: any, reject) => {
...
class AppHelper {
    ...
    setCookie(cname: string, cvalue: string, date: Date | number) {
        let d = new Date();
    ...
    get_cached_column(key: any, gridfilter: Array<GridFilter>) {
        let cols = this.get_localstorage(key);
        if (cols) {
            gridfilter.map(d => {
                let item = cols.find((x: { col: string; }) => x.col == d.ColumnName);
