...
export class commongrid_Component {
    ...
    @ViewChild('currentTarget') currentTarget : any;
    ...
    constructor(public service: SystemService, public eRef: ElementRef, public changeDetector: ChangeDetectorRef, public router: Router) {
        ...
        this.service.App.refreshGrid.subscribe(() => {
    ...
    changeSorting(columnName: string, Type: string, isFilter: boolean = false) {
        ...
        var sortFunc = function (field: string, rev: boolean, primer: ((a: any) => number) | ((a: any) => string)) {
        ...
        var pathValue = function (obj: { [x: string]: any; }, path: string ) {
           let path_temp = path.split('.');
           let len = path.length;
            for (var i = 0;  i < len; i++) {
               // obj = obj[path[i]];
                obj = obj[path_temp[i]];
        ...
        var primer = isNumeric ?
            ...
            function (a: any) { return String(a).toUpperCase(); };
    ...
    DateChange(instance: { target: { value: any; }; }, filter: { Value: any; }) {
    ...
    ActionClick(item: any, action: any) {
